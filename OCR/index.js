...
exports.translateText = (event) => {
  ...
  return Promise.resolve()
    ...
    .then(([translation]) => {
      ...
      return publishResult(config.receiptText, messageData);
    })
    .then(() => {
...
exports.saveResult = (event) => {
  ...
  return Promise.resolve()
    .then(() => {
      ...
      const bucketName = config.divvy_up_receipt_text;
      const filename = renameImageForSave(payload.filename, payload.lang);
      const file = storage.bucket(divvy_up_receipt_text).file(filename);
